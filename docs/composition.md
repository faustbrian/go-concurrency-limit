...
contracts with Retry v1.1.0's strict policy and execution surfaces and the
repository hedge module. Its in-process operations report known outcomes. The
harness proves a local admission rejection is not retried, each hedge attempt
crosses admission, a rejected hedge does not invoke downstream work or become a
learning sample, and all permits and the shared hedge budget are released.
